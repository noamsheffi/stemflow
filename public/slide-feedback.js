(function () {
  ...
  function paintSelection(type) { var item = types.find(function (entry) { return entry[0] === type; }); button.textContent = item ? item[2] : '?'; button.dataset.selected = item ? 'true' : 'false'; button.setAttribute('aria-label', item ? 'המשוב שסימנת: ' + item[1] : 'משוב על השקף הנוכחי'); button.title = item ? 'סימנת: ' + item[1] : 'משוב על השקף הנוכחי'; options.querySelectorAll('button').forEach(function (option) { option.classList.toggle('selected', option.dataset.type === type); }); selected = type || null; text.hidden = selected !== 'QUESTION' && selected !== 'POSSIBLE_ERROR'; submit.disabled = !selected; }
  function syncSlideSelection() { var key = keyFor(currentSlide()); if (!key || key === currentKey) return; currentKey = key; status.textContent = ''; status.className = 'syllo-feedback-status'; text.value = ''; paintSelection(marksBySlide[key]); }
  types.forEach(function (item) { var option = document.createElement('button'); option.type = 'button'; option.textContent = item[1]; option.dataset.type = item[0]; option.addEventListener('click', function () { if (currentKey) marksBySlide[currentKey] = item[0]; paintSelection(item[0]); status.textContent = ''; status.className = 'syllo-feedback-status'; }); options.appendChild(option); });
  ...
  submit.addEventListener('click', function () {
    ...
    var slide = currentSlide(); if (!slide) { status.className = 'syllo-feedback-status error'; status.textContent = 'לא זוהה שקף פעיל. עברו לשקף ונסו שוב.'; return; }
    ...
    fetch('/api/student/slide-feedback', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload), keepalive: true }).then(function (response) { if (!response.ok) throw new Error(response.status === 400 ? 'פרטי השקף לא תקינים. רעננו את המערך ונסו שוב.' : response.status === 503 ? 'השמירה אינה זמינה כרגע. נסו שוב בעוד רגע.' : 'לא הצלחנו לשמור את המשוב. נסו שוב.');
      ...
      status.className = 'syllo-feedback-status success'; status.textContent = '✓ נשלח'; setTimeout(function () { panel.hidden = true; status.textContent = ''; status.className = 'syllo-feedback-status'; lastSubmit = ''; text.value = ''; }, 1100);
  ...
  var style = document.createElement('style'); style.textContent = `
    ...
    .syllo-feedback-trigger{position:fixed;top:20px;right:20px;left:auto;z-index:2147483647;display:grid;place-items:center;width:46px;height:46px;padding:0;border:1px solid #cbd5e1;border-radius:50%;background:#fff;color:#17202a;box-shadow:0 5px 18px #0f172a24;font:700 20px/1 system-ui;cursor:pointer;transition:background .16s,color .16s,transform .16s}
    ...
    .syllo-feedback-panel{position:fixed;top:76px;right:20px;left:auto;z-index:2147483647;display:flex;flex-direction:column;width:min(360px,calc(100vw - 24px));max-height:calc(100vh - 92px);overflow:hidden;border:1px solid #dce3eb;border-radius:20px;background:#fff;color:#17202a;box-shadow:0 20px 60px #17202a30;font:14px/1.5 system-ui,-apple-system,sans-serif}
    ...
    @media(max-width:520px){.syllo-feedback-trigger{top:14px;right:14px}.syllo-feedback-panel{top:68px;right:12px;width:calc(100vw - 24px);max-height:calc(100vh - 80px)}}

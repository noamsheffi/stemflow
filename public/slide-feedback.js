(function () {
  ...
  submit.addEventListener('click', function () {
    ...
    fetch('/api/student/slide-feedback', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload), keepalive: true }).then(function (response) { if (!response.ok) throw new Error('failed');
      ...
      status.textContent = '✓ נשלח'; setTimeout(function () { panel.hidden = true; status.textContent = ''; selected = null; lastSubmit = ''; submit.disabled = true; options.querySelectorAll('button').forEach(function (b) { b.classList.remove('selected'); }); text.value = ''; }, 850);
    }).catch(function () { status.textContent = 'לא הצלחנו לשלוח'; submit.disabled = false; lastSubmit = ''; });
  });
  var style = document.createElement('style'); style.textContent = '.syllo-feedback-trigger{position:fixed;top:20px;left:20px;z-index:10001;width:34px;height:34px;border:1px solid currentColor;border-radius:50%;background:rgba(255,255,255,.9);font-weight:700;cursor:pointer}.syllo-feedback-panel{position:fixed;top:62px;left:20px;z-index:10001;width:230px;padding:14px;background:#fff;color:#1a1a1a;border:1px solid #ddd;box-shadow:0 8px 24px #0002;font:14px system-ui}.syllo-feedback-options{display:grid;gap:6px;margin:10px 0}.syllo-feedback-options button,.syllo-feedback-submit{padding:7px;border:1px solid #bbb;background:#fafafa;cursor:pointer}.syllo-feedback-options button.selected{background:#dff5ea;border-color:#008f4d}.syllo-feedback-panel textarea{width:100%;margin:2px 0 8px;min-height:58px;resize:vertical}.syllo-feedback-submit:disabled{opacity:.5;cursor:default}.syllo-feedback-status{display:block;margin-top:8px;color:#008f4d;font-weight:700}'; document.head.appendChild(style); body.appendChild(button); body.appendChild(panel);
}());

...
import { ACCESS_COOKIE, SESSION_SECONDS, createAccessSession, safeReturnPath, validAccessCode } from "../../../../lib/access-session";
...
export async function POST(request: NextRequest) {
  ...
  if (typeof code !== "string" || code.length > 128 || !validAccessCode(code)) {
    login.searchParams.set("error", "invalid");
    return NextResponse.redirect(login, 303);
  }
  let token: string;
  try { token = createAccessSession(); } catch {

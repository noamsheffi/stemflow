...
import { lessons, lessonHref, getFormula } from "../lib/course-data";
...
export default function ConceptDetails({ concept }: { concept: typeof conceptMap[number] }) {
  ...
  return <div className={styles.details}>
    ...
    {concept.formulas.length > 0 && <section><h3>הנוסחאות שמאחורי המושג</h3>{concept.formulas.map((tex) => <div className={styles.math} key={tex}><FormulaMath tex={tex} display /></div>)}<div className={styles.links}>{(conceptFormulaIds[concept.id] ?? []).map((id) => { const formula = getFormula(id); return formula ? <Link key={id} href={`/formulas/${id}`}>{formula.name} ←</Link> : null; })}</div></section>}

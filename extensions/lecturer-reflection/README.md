...
Load this folder as an unpacked Chromium extension (Extensions → Developer mode → Load unpacked). Opening a supported Syllo presentation URL (`/courses/communication-systems/lesson-*`) starts a session automatically; lesson lists and course pages do not show the toolbar. The small timer confirms it is active. Click the extension action again to open review. Auto-start is limited to supported lesson presentations on `localhost:3000` and the configured Syllo domains, never every website. Enable file URL access in extension settings if opening local HTML files; local files still need a click on the extension action.
